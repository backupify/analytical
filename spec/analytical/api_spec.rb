...
describe "Analytical::Api" do
  ...
  describe 'on initialization' do
    it 'should construct an api class for each module' do
      Analytical::Modules::Console.should_receive(:new).and_return(@console = double('console'))
      Analytical::Modules::Google.should_receive(:new).and_return(@google = double('google'))
    ...
    it 'should pass the ssl option on to the module constructor' do
      Analytical::Modules::Console.should_receive(:new).with(hash_including(:ssl=>true)).and_return(@console = double('console'))
    ...
    describe 'with a session option' do
      ...
      it 'should create a new SessionCommandStore for each module' do
        Analytical::SessionCommandStore.should_receive(:new).with(@session, :console).and_return(@console_store = double('console_store'))
        Analytical::SessionCommandStore.should_receive(:new).with(@session, :google).and_return(@google_store = double('google_store'))        
        Analytical::Modules::Console.should_receive(:new).with(:session_store=>@console_store, :session=>@session).and_return(double('console'))
        Analytical::Modules::Google.should_receive(:new).with(:session_store=>@google_store, :session=>@session).and_return(double('google'))
  ...
  describe 'with modules' do
    before(:each) do
      Analytical::Modules::Console.stub(:new).and_return(@console = double('console'))
      Analytical::Modules::Google.stub(:new).and_return(@google = double('google'))
      Analytical::Modules::Clicky.stub(:new).and_return(@clicky = double('clicky'))
      Analytical::Modules::Chartbeat.stub(:new).and_return(@chartbeat = double('chartbeat'))
